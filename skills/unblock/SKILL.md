...
1. Run `${CLAUDE_PLUGIN_ROOT}/scripts/task.sh diagnose`. It prints one global `verify:` color plus, per stuck item, `commits=`, `review=`, `faillog=`, and the NEEDS_HUMAN `reason:`. `(nothing blocked or in-progress)` → say so and stop. If a task id was given, act only on it.
