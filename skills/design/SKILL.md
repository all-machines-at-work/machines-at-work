...
Write `intentpipe/tasks/<id>-<slug>/design.md` for task $ARGUMENTS. The implementer will follow it literally — be concrete, not aspirational. If the task's `Resources:` field lists files, Read them first (images too): a referenced mockup is the design's starting point — interpret it, don't override it.
