...
Replace `intentpipe/specs/` and its `specs/updates/` subfolder with a single flat folder of intent notes -
`intentpipe/updates/` (sibling of `tasks/`). No living document to maintain, no subfolder nesting. A note is
...
- `skills/plan/SKILL.md` — reads `intentpipe/updates/` (ignores `README.md`), drops the absorb-into-spec step
  and spec-diff approval (the human approves the task plan), records `Intent:`, removes planned notes.
- `skills/init-project/SKILL.md` — creates `intentpipe/updates/README.md` (not `specs/spec.md`); onboarding
...
Migration (project mid-flight): rename `intentpipe/specs/updates/*` up to `intentpipe/updates/`, drop
`intentpipe/specs/spec.md`; leave old tasks' `Spec:` fields as historical (`/retro` reads whichever field a
