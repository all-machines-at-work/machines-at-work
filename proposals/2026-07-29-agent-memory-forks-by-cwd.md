...
Proposal — **apply by hand**: `machines-at-work/scripts/{loop.sh,preflight.sh}` and
...
| `bibbles/machines-at-work/.claude/agent-memory/` | 2026-07-24 → 2026-07-27 | implementer: 5 topics; reviewer: 4 topics |
| `bibbles/machines-at-work/tasks/0104-…/.claude/agent-memory/` | 2026-07-27 | implementer: 1 topic, `type: feedback` |
| `machines-at-work/tasks/.claude/agent-memory/` | — | empty |
| `machines-at-work/tasks/0108-…/.claude/agent-memory/` | — | empty |
...
- The **`machines-at-work/`** store holds every lesson learned since the Telegram triggers went live:
  ...
  An interactive `/machines-at-work:build` started at the project root today loads **none** of them.
...
- `scripts/lib.sh:7-16` — `find_workspace()` deliberately walks up from `$PWD`, so every script works
  identically from the project root, from `machines-at-work/`, or from inside a repo. Correct for the
...
- `server-orchestrator/daemon.py` — `dispatch()` does
  ...
  `"workspace": "/home/agent/projects/bibbles/machines-at-work"`. So **every** 🚀 / 🧠 / 🩹 run has
  cwd = `machines-at-work/`, while a human's interactive session sits at the project root. The fork
...
+        # workspace is <root>/machines-at-work, so spawning there gives headless runs a
+        # different memory store than an interactive session at the project root.
+        root = os.path.dirname(workspace) if os.path.basename(workspace) == "machines-at-work" else workspace
...
- `cd` in `loop.sh` changes cwd for everything the loop invokes. Audited: `loop.sh` and `lib.sh` use
  ...
  `machines-at-work/`-relative cwd would — the template's own example already passes
  `"$MAW_WORKSPACE"` explicitly.
- The daemon change is a no-op for any registry entry not ending in `machines-at-work`.
