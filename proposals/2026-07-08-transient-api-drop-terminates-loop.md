...
Proposal — **apply by hand in the intentpipe repo** (`intentpipe/scripts/{loop.sh,lib.sh}`);
...
1. **It's routed into bounded auto-resume, not transient-retry.** The transient-retry logic (from the sibling
   retro) only fires when `status = todo`. But `/intentpipe:build` runs `task.sh start` early, so by the drop
