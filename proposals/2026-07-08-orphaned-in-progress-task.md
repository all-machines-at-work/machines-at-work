...
Proposal — **apply by hand in the intentpipe repo** (`intentpipe/scripts/{loop.sh,task.sh}`);
...
- A fresh loop invocation then ran `task.sh next`, which returned **0038** — skipping the orphaned
  in-progress 0037 — and began `/intentpipe:build 0038`. **0038 `Depends on 0037`**, so it was building a
